...
class TemporalNetworkBuilder(WikipediaNetworkBuilder):
    ...
    def _create_snapshot(self, metadata: Dict = None) -> NetworkSnapshot:
        ...
        snapshot = NetworkSnapshot(
            ...
            node_attributes={
                node: dict(self.graph.nodes[node]) for node in self.graph.nodes()
            },
            edge_attributes={
                edge: dict(self.graph.edges[edge]) for edge in self.graph.edges()
            },
    ...
    def _track_node_addition(self, node: str, depth: int = 0, source: str = None):
        ...
        if node not in self.node_addition_times:
            ...
            self.graph.nodes[node]["added_at"] = self.node_addition_times[node][
                "timestamp"
            ]
            self.graph.nodes[node]["discovery_order"] = self.node_addition_times[node][
                "order"
            ]
    ...
    async def build_network_breadth_first_async(
        ...
        self._create_snapshot(
            {"type": "initial", "seeds": seeds, "nodes_so_far": 0, "edges_so_far": 0}
        )

        # Create a wrapper progress callback that also creates snapshots
        def temporal_progress_callback(
            articles_processed: int, total_articles: int = None
        ):
            # Create snapshot if needed
            if self._should_create_snapshot():
                self._create_snapshot(
        ...
        result = await super().build_network_breadth_first_async(
            seeds, temporal_progress_callback
        )
    ...
    def save_to_history(self, history_dir: str = "outputs/history"):
        ...
        with open(metadata_path, "w") as f:
            ...
            f.write(
                f"Network size: {len(self.graph.nodes())} nodes, {len(self.graph.edges())} edges\n"
            )
    ...
    def create_evolution_visualization(
        self, output_path: str = "network_evolution.gif", **kwargs
    ) -> str:
    ...
    def create_evolution_dashboard(
        self, output_path: str = "evolution_dashboard.png"
    ) -> str:
    ...
    def _analyze_node_addition_pattern(self) -> Dict:
        ...
        start_time = min(
            info["timestamp"] for info in self.node_addition_times.values()
        )
        ...
        return {
            ...
            "peak_discovery_interval": (
                max(addition_rates.keys(), key=addition_rates.get)
                if addition_rates
                else None
            ),
            "total_discovery_time": (
                max(addition_rates.keys()) * 10 if addition_rates else 0
            ),
        }
    ...
    def _analyze_discovery_depths(self) -> Dict:
        ...
        return {
            ...
            "depth_distribution": {
                depth: len(nodes) for depth, nodes in depth_analysis.items()
            },
    ...
    def export_temporal_data(
        self, output_path: str = "temporal_network_data.json"
    ) -> str:
    ...
    def create_growth_animation_with_metrics(
        self, output_path: str = "growth_with_metrics.gif"
    ) -> str:
        ...
        def animate(frame):
            ...
            if len(graph.nodes()) > 0:
                ...
                for node in graph.nodes():
                    ...
                    max_order = max(
                        (
                            info.get("order", 0)
                            for info in self.node_addition_times.values()
                        ),
                        default=0,
                    )
                ...
                nx.draw_networkx_nodes(
                    graph, pos, node_color=node_colors, node_size=50, alpha=0.8, ax=ax1
                )
                nx.draw_networkx_edges(
                    graph, pos, edge_color="gray", alpha=0.5, width=0.5, ax=ax1
                )
            ...
            ax2.plot(
                range(current_frame), node_counts[:current_frame], "b-o", label="Nodes"
            )
            ax2.plot(
                range(current_frame), edge_counts[:current_frame], "r-s", label="Edges"
            )
    ...
    def create_evolution_dashboard(
        self, output_path: str = "temporal_network_dashboard.png"
    ) -> str:
    ...
    def create_evolution_visualization(
        self,
        output_path: str = "temporal_network_evolution.gif",
        color_by: str = "growth",
        fps: int = 2,
    ) -> str:
        """Create animated visualization of network evolution."""
...
def demo_temporal_network_building():
    ...
    config = NetworkConfig(max_articles_to_process=30, max_depth=2, links_per_article=5)
    ...
    graph = builder.build_network(
        ["Machine Learning", "Data Science"], method="breadth_first"
    )
